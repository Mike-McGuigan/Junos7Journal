...
import json, sys
ROOT=Path(__file__).resolve().parents[1]; VERSION="2.0.0"; RELEASE="Production Dashboard"
def backup(path):
    if path.exists(): path.with_suffix(path.suffix+".bak-2-0-0").write_text(path.read_text(encoding="utf-8"),encoding="utf-8")
def load_update(path):
    data=json.loads(path.read_text(encoding="utf-8"))
    if "routePoint" not in data or "tracker" not in data: raise SystemExit("Invalid manual location update.")
    data["routePoint"]["phase"]="current"; return data
def normalise_route(route,new_point):
    cleaned=[]
    for item in route:
        if item.get("name")==new_point.get("name") and item.get("date")==new_point.get("date"): continue
        if item.get("phase")=="current": item["phase"]="onboard"
        cleaned.append(item)
    new_point["phase"]="current"; cleaned.append(new_point); return cleaned
def update_route(path,point):
    if not path.exists(): return False
    route=normalise_route(json.loads(path.read_text(encoding="utf-8")),point); backup(path); path.write_text(json.dumps(route,indent=2),encoding="utf-8"); return True
def update_dashboard(path,update):
    ...
    tracker["latestSource"]=manual.get("source","captains dashboard"); tracker["latestPrecisePosition"]=pos if "precise" in pos.get("precision","") else None; tracker["latestApproximatePosition"]=pos; tracker["note"]=update["routePoint"].get("note","")
...
def write_record(update):
    ts=update["tracker"]["timestampUtc"].replace(":","").replace("-",""); name=update["routePoint"]["name"].lower().replace(" ","-").replace("/","-")
...
def main():
    ...
    update=load_update(Path(sys.argv[1])); changed=[]
    ...
    if route_path.exists():
        ...
        if count!=1: raise SystemExit(f"Expected exactly one current marker, found {count}.")
    print("Manual location applied."); print(f"Location: {update['routePoint']['name']}"); print(f"Coordinates: {update['routePoint']['lat']}, {update['routePoint']['lng']}"); print(f"Tracker record: {record.relative_to(ROOT)}"); print("Updated files where present:")
