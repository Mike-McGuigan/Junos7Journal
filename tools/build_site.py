...
def main():
    ...
    info={"version":version,"release":"Story Engine","buildUtc":build_utc,"project":"Juno's 7 Mediterranean Journal","pagesSource":"docs"}
    (data_dir/"version.json").write_text(json.dumps(info,indent=2),encoding="utf-8")
    (DOCS/"build-info.json").write_text(json.dumps(info,indent=2),encoding="utf-8")
    (DOCS/"build-info.html").write_text(f'<!DOCTYPE html><html><head><meta charset="utf-8"><title>Build Info</title><link rel="stylesheet" href="assets/css/style.css"></head><body><section class="section"><p class="kicker">Build Info</p><h2>Juno\'s 7 Mediterranean Journal</h2><p><strong>Version:</strong> {version}</p><p><strong>Release:</strong> Story Engine</p><p><strong>Built:</strong> {build_utc}</p><p><a href="index.html">Home</a> · <a href="voyage.html">Voyage dashboard</a></p></section></body></html>',encoding="utf-8")
    manifest=[]
    for path in DOCS.rglob("*"):
        if path.is_file(): manifest.append({"path":path.relative_to(DOCS).as_posix(),"sizeBytes":path.stat().st_size,"sha256":sha256(path)})
    (DOCS/"build-manifest.json").write_text(json.dumps({"version":version,"release":"Story Engine","buildUtc":build_utc,"fileCount":len(manifest),"files":manifest}, indent=2),encoding="utf-8")
