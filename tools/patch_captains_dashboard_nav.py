...
FILES=[ROOT/'site'/'index.html',ROOT/'site'/'voyage.html']
def patch(path):
    if not path.exists(): return False
    text=path.read_text(encoding='utf-8'); original=text
    path.with_suffix(path.suffix+'.bak-1-2-0-dashboard-nav').write_text(text,encoding='utf-8')
    text=text.replace('Manual Location',"Captain's Dashboard")
...
def main():
    ...
    print("Captain's Dashboard nav patched:")
